...
os.makedirs(dire+"/pretext_supervised/",exist_ok=True)
...
for file in tqdm(pretext_files):
    ...
    for i in range(half_window,x_dat.shape[0]-half_window):
        dct = {}
        temp_path = os.path.join(dire+"/pretext_supervised/",str(cnt)+".npz")
...
os.makedirs(dire+"/test_supervised/",exist_ok=True)
...
for file in tqdm(test_files):
    ...
    temp_path = os.path.join(dire+"/test_supervised/",os.path.basename(file))
