...
train_save_path = '/scratch/allsamples_tuh/train_shhs_1_per'
test_save_path = '/scratch/allsamples_tuh/test_shhs'
...
def multi_fc(save_path,ds,start,end):
    for i in tqdm(range(start,end)):
        sub = ds[i]
        dct = {}
        temp_path = os.path.join(save_path, str(i) +'.npz')
        dct['x'] = torch.nn.functional.interpolate(torch.tensor(sub[0]).unsqueeze(0),scale_factor=3750/7500).numpy().squeeze(0)
        dct['y'] = sub[1]
        np.savez(temp_path, **dct)

multi_fc(train_save_path,train_dataset,0,len(train_dataset)//100)
multi_fc(test_save_path,eval_dataset,0,len(eval_dataset))
