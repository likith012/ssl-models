...
def evaluate(q_encoder, train_loader, test_loader,device, i):
    ...
    with torch.no_grad():
        for (X_val, y_val) in train_loader:
            ...
            emb_val.extend(q_encoder(X_val)[0].cpu().tolist())
    ...
    with torch.no_grad():
        for (X_test, y_test) in test_loader:
            ...
            emb_test.extend(q_encoder(X_test)[0].cpu().tolist())
...
def task(X_train, X_test, y_train, y_test,i):
    ...
    lin_trainer = pl.Trainer(callbacks=[early_stop_callback], gpus=1, precision=16, num_sanity_val_steps=0, enable_checkpointing=False, max_epochs=1, auto_lr_find=True)
    ...
    for epoch in range(Epoch):
        ...
        for index, (aug1, aug2) in enumerate(
            ...
            with torch.cuda.amp.autocast():           
                ...
                loss = q_encoder(aug1[:, i].unsqueeze(1),aug2[:,i].unsqueeze(1),epoch) #(B, 128)
        ...
        if epoch >=0 and (epoch) % 1 == 0:
            ...
            test_acc, test_f1, test_kappa, bal_acc = kfold_evaluate(
                q_encoder.model.eeg_encoder, test_subjects, device, BATCH_SIZE
            )
            ...
            if test_f1 > best_f1:   
                best_f1 = test_f1
                torch.save(q_encoder.model.eeg_encoder.state_dict(), SAVE_PATH)
