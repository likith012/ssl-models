...
def evaluate(q_encoder, train_loader, test_loader, device, i):
    ...
    with torch.no_grad():
        for (X_val, y_val) in train_loader:
            ...
            emb_val.extend(q_encoder(X_val[:, :1, :]).cpu().tolist())
    ...
    with torch.no_grad():
        for (X_test, y_test) in test_loader:
            ...
            emb_test.extend(q_encoder(X_test[:, :1, :] ).cpu().tolist())
...
def task(X_train, X_test, y_train, y_test, i):
    ...
    train = DataLoader(train, batch_size=256, shuffle=True,num_workers=6)
